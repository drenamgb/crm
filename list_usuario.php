...
try{
    ...
    if($_SESSION['perfil']=="root"){
        $stmt = $PDO->prepare("SELECT * FROM view_usuario");
        $codHidden = "";
    }else{
        $stmt = $PDO->prepare("call select_usuario(:idEmpresa)");
...
<div id="container">
    ...
    <table class="table table-hover table-striped small">
        ...
        for($i=0;$i<$total;$i++){
            ...
            echo "<td>{$retorno[$i]['login']}</td>";
            echo "<td>{$retorno[$i]['perfil']}</td>";
            echo "<td>".number_format($retorno[$i]['meta'],2,",",".")."</td>";
